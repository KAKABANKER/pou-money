...
const JWT_SECRET = 'pou_money_secret_2025';
...
app.post('/api/cadastrar', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: result.rows[0].id, email, is_admin: false }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/verificar', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('SELECT id, nome, email, saldo, moedas, pontos, nivel, is_admin FROM users WHERE id = $1', [decoded.id]);
...
app.post('/api/salvar-pontuacao', async (req, res) => {
    ...
    try {
        ...
        await pool.query('UPDATE users SET moedas = moedas + $1, pontos = pontos + $2 WHERE id = $3', [moedas || 0, pontuacao, decoded.id]);
...
app.post('/api/criar-deposito', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, payment: { qr_code: mockQrCode, code: mockPixCode, value: valor } });
    } catch (err) {
...
app.get('/api/ranking', async (req, res) => {
    try {
        const result = await pool.query('SELECT id, nome, pontos, nivel, moedas FROM users WHERE ativo = true AND is_admin = false ORDER BY pontos DESC LIMIT 100');
...
initDB().then(() => {
    app.listen(PORT, '0.0.0.0', () => {
        ...
        console.log(`👤 Admin: admin@poumoney.com / admin123`);
    });
});